// The `screencapturekit` dependency links a Swift bridge that loads
// `@rpath/libswift_Concurrency.dylib` at runtime. Its build script emits
// rpath link args, but `cargo:rustc-link-arg` from a dependency only applies
// to that dependency's own targets, so the rpath must be baked in here for
// this package's binaries and tests to load on hosts where the Swift
// concurrency runtime ships only inside the Xcode toolchain.
